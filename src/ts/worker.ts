  ...
  if ('dst' in event.data)
    switch (event.data.dst) {
      ...
      case 'size':
        ...
        break;
    } else postMessage("yes sir");
};

(self as any).getKey = async () => {
  self.postMessage({ dst: "kbd" });
...
const interval = setInterval(async () => await kbfifo.put("ёпт"), 250);
...
async function runWasm() {
  ...
  const Module = await createWasmModule({
    locateFile: () => { return "/js/mod/module.wasm" },
    // preRun: [],
    'print': (text: string) => {
      if (arguments.length > 1) text = Array.prototype.slice.call(arguments).join(' ');
      self.postMessage({ dst: "out", text: text, });
    },
  });
