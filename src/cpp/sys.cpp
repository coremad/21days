...
void kol_exit() { asm volatile("int $0x40" ::"a"(-1)); }
...
void (*__attribute__((stdcall)) con_init)(unsigned w_w, unsigned w_h,
...
void *kol_cofflib_procload(kol_struct_import *imp, char *name) {
  for (int i = 0;; i++)
    if (NULL == ((imp + i)->name))
      break;
    else if (0 == strcmp(name, (imp + i)->name))
      return (imp + i)->data;
  return NULL;
}
...
void CONSOLE_INIT(char *title) {
  ...
  con_init = (__attribute__((stdcall)) void (*)(
      unsigned, unsigned, unsigned, unsigned,
      const char *))kol_cofflib_procload(imp, "con_init");
  if (con_init == NULL)
    kol_exit();

  printf2 = (__attribute__((cdecl)) void (*)(
      const char *, ...))kol_cofflib_procload(imp, "con_printf");
  if (printf2 == NULL)
    kol_exit();

  con_exit = (__attribute__((stdcall)) void (*)(char))kol_cofflib_procload(
      imp, "con_exit");
  if (con_exit == NULL)
    kol_exit();

  getch = (__attribute__((stdcall)) int (*)())kol_cofflib_procload(imp,
                                                                   "con_getch");
  if (getch == NULL)
    kol_exit();

  cls =
      (__attribute__((stdcall)) void (*)())kol_cofflib_procload(imp, "con_cls");
  if (cls == NULL)
    kol_exit();

  kbhit = (__attribute__((stdcall)) int (*)())kol_cofflib_procload(imp,
                                                                   "con_kbhit");
  if (kbhit == NULL)
    kol_exit();

  con_set_cursor_pos = (__attribute__((stdcall)) void (*)(
      int, int))kol_cofflib_procload(imp, "con_set_cursor_pos");
  if (con_set_cursor_pos == NULL)
    kol_exit();

  con_set_cursor_height = (__attribute__((stdcall)) int (*)(
      int))kol_cofflib_procload(imp, "con_set_cursor_height");
...
void hideCursor() { con_set_cursor_height(0); }
...
string txt(string s, string s2, string s3) { return txt(txt(s, s2), s3); }
string txt(string s, char ch, string s2) { return txt(txt(s, ch), s2); }
string txt(string s, char ch, string s2, char ch2) {
  return txt(txt(txt(s, ch), s2), ch2);
}
string txt(string s, int d1, int d2) { return txt(txt(s, d1), d2); }
...
void initConsole() { CONSOLE_INIT(windowTitle); }
...
void hideCursor() { printf2("\033[?25l"); }

void showCursor() { printf2("\033[?25h"); }

void consoleGoto(int line, int column) { printf2("\033[%d;%dH", line, column); }
...
void initConsole() { printf2("\033]0;%s\007", windowTitle); }
#elif defined _WIN32
#define printf2 printf
#include <cstdlib> // system
#include <stdio.h> // printf, vsprintf
...
int jgetch();
void jgetWinWH();
// void jconsoleGoto(int line, int left);
// void jprint2(char &s);
// void jcls();
}
...
std::string txt(const char *s, ...) {
  va_list args;
  va_start(args, s);
  vsprintf(chrbuf, s, args);
  va_end(args);
  std::string str(chrbuf);
...
void consoleGoto(int line, int left) {
  // jconsoleGoto(line, left);
  printf2("\033[%d;%dH", line, left);
}
...
void initTermios() {
  //   tcgetattr(0, &old);           /* grab old terminal i/o settings */
  //   new1 = old;                   /* make new settings same as old settings
  //   */ new1.c_lflag &= ~ICANON;      /* disable buffered i/o */ new1.c_lflag
  //   &= ~ECHO;        /* set echo mode */ tcsetattr(0, TCSANOW, &new1); /* use
  //   these new terminal i/o settings now */
}
...
EM_ASYNC_JS(char, checkKey, (), { return await self.getKey(); });
...
void CP866ArrayToString();
void mprintChar();
void jinit(void *, void *);
void jend();
